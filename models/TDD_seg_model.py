...
class TDD_seg_model(nn.Module):
    def __init__(self, cfg={'Attention_order': "LGLGL",     
       ...
       'loss_ratio': [0.5, 0.5, 0.5, 1, 1, 1]}, n_channels=3,pool_num=4):
        super(TDD_seg_model, self).__init__()
        self.msgms_loss = MSGMS_Loss(pool_num=pool_num)
    ...
    def forward(self, *input):
        ...
        if not test_mode:
            for i in range(6): 
                ...
                dec_loss = self.msgms_loss(pred[5 - i].squeeze(1), tar.squeeze(1))
                loss += dec_loss * self.cfg['loss_ratio'][5 - i]
...
class GAM(nn.Module):
    def __init__(self, in_channel):
        ...
        self.multihead_attn = nn.MultiheadAttention(in_channel, 1, batch_first=True) # 创建多头注意力机制
