        ...
        for y in range(0, height, stride):
            ...
            if patch_id in patch_id_list:
                ...
                if only_crop_image:
                    ...
                    write_img(img=label_patch,
                              save_path=os.path.join(label_save_path, patch_id + '.' + label_extension), )
...
def main(config: dict):
    ...
    if single_image:
        ...
        for i in tqdm(range(len(img_name_list))):
            ...
            label_name = img_id.replace(filename_difference['img'],
                                        filename_difference['label']) + '.' + label_extension
...
if __name__ == '__main__':
    ...
    img_folder_path = r"D:/PapersCode/TDD-master-zt/data/ABU/abu-urban-1.img"  # single image path or folder path
    label_folder_path = 'D:/PapersCode/TDD-master-zt/data/ABU/abu-urban-1-mask.img'  # single label path or folder path
    save_path = r"D:\PapersCode\TDD-master-zt\data\ABU\training_patches"  # saved path of cropped patches
    patch_size = 50
    stride = 25
