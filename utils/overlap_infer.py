...
def pad_borders(img, tile_size, padding_mode):
    assert padding_mode in ['constant', 'mirror']  # 填充模式
...
def batch_predict(img_list, model, device, test_size):
    ...
    with torch.no_grad():
        ...
        img_list_after_transforms = [torch.unsqueeze(torch.from_numpy(img).permute(2, 0, 1), dim=0) for img in
                                     resized_imgs]
        ...
        resized_img = resize_transform2(image=output[5].permute((2, 3, 0, 1)).detach().cpu().numpy()[:, :, 0, 0])[
            'image']
